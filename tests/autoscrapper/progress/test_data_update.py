...
def test_fetch_supabase_all_missing_key():
    with mock.patch("autoscrapper.progress.data_update.SUPABASE_ANON_KEY", None):
        with pytest.raises(DownloadError, match="METAFORGE_SUPABASE_ANON_KEY environment variable is not set"):
...
def test_update_data_snapshot_handles_missing_key(tmp_path):
    ...
    with mock.patch("autoscrapper.progress.data_update._fetch_all_items", return_value=[]), \
        ...
        assert "METAFORGE_SUPABASE_ANON_KEY environment variable is not set" in str(args[1])
