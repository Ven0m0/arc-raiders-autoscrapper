...
Apply tiered loading based on file size:
...
- If full understanding is needed, delegate to an Explore subagent
...
- Prefer StrReplace which requires reading only the old_string portion
...
- Reading 10 full 500-line files consumes 50-100K tokens (25-50% of budget)
- Reading targeted 50-line sections from 10 files consumes 5-10K tokens (2.5-5% of budget)
...
- `cat large_file` - unbounded read
- `git diff` without `--stat` - large diffs
- `git log` without `-n` - entire history

Use bounded equivalents:

- `head -n 100 large_file`
- `git diff --stat` then targeted reads
- `git log -n 20`
