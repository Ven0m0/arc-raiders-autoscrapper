...
- **REPLs**: `python`, `node`, `irb` (without script/command)
...
2. Check `--help` to discover non-interactive options: `cmd --help | grep -i "non-interactive\|force\|yes"`.
3. Prefer Kilo file tools (`read`, `edit`, `write`) over shell commands for file operations.
4. Set a timeout for any command that might unexpectedly prompt.
