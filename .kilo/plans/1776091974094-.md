...
1. **API Key Source**: Should the app register for its own app key and distribute it, or should each user register their own app?
   - **Recommendation**: Register an official app key for "Arc Raiders AutoScrapper" and include it in the app. Users only need to generate a user key.

2. **Fallback Behavior**: If API sync is enabled but fails (rate limited, keys invalid), should it:
   - a) Fall back to OCR automatically
   - b) Show error and require user to retry or switch modes
   - **Recommendation**: Option (b) - explicit error message with option to use OCR instead

3. **Data Freshness**: How should we handle the fact that API data might be stale (synced from game at unknown time)?
   - **Recommendation**: Show "Last synced: X minutes ago" warning if data is >1 hour old

4. **Item Name Mapping**: The API returns item IDs (e.g., "advanced-arc-powercell") but the OCR returns display names (e.g., "Advanced ARC Powercell"). How to reconcile?
